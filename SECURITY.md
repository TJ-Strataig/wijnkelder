...
Modern en Klassiek delen dezelfde router en autorisatie. De mobiele onderbalk en desktop-zijbalk leiden via **Meer** naar Selecties, Avondplanning, Blindproeven, Rekken & vakken, Herkomst, Wijnhuizen en Instellingen; oude routes worden alleen als compatibiliteitsalias afgehandeld. Selecties, avondplanningen en blindproeverijen zijn huishoudgegevens en volgen dezelfde sessie- en eigenaarscontroles als de collectie. Rekken en vakken onthullen alleen locaties aan ingelogde huishoudleden. De vrienden-scan schrijft pas na bevestiging een historie-item en maakt geen voorraad- of aankooprecord.

De huidige testset gebruikt Node 24 en `npm ci` met de bestaande lockfile. De 117 tests bestrijken API-regels, releaseselectie, ontwerpmodus en projectintegriteit. Succesvolle registraties worden alleen in het afzonderlijke `registration.test.mjs` gesimuleerd via een modulemock van de WebAuthn-verifier; daarbij worden challenge, origin, RP ID en vereiste gebruikersverificatie gecontroleerd. De beveiligingstests gebruiken de echte verifier en bewaken dat ongeldige registraties geen gebruiker, credential of sessie aanmaken. Productieauthenticatie is hiervoor niet aangepast.
...
De migraties `0006_selecties.sql`, `0007_avonden.sql`, `0008_blindproeven.sql` en `0009_rekken_vakken.sql` worden uitsluitend in die volgorde en na controle van de D1-status uitgevoerd. GitHub Actions voert **geen automatische DB-migraties** uit; de workflow doet alleen tests en een Worker-dry-run. De JSON-export is geen volledige back-up: passkeys, sessies, instellingen, chat, push-abonnementen en R2-foto's vallen buiten de export. Exporteer daarom alleen naar een afgeschermde locatie en deel geen bestanden met locatie-, aankoop- of proefnotities onnodig.

De PWA-shell kan zonder netwerk openen, maar API-data, uploads, scans, passkeys en mutaties werken alleen online. Browserondersteuning voor camera, microfoon en WebAuthn verschilt per apparaat. Er zijn geen realtime updates of automatische conflictresolutie; AI-uitvoer blijft onbetrouwbare invoer die de gebruiker moet controleren.
